...
class Soa_model extends CI_Model {
	...
	public function get_soa_per_customer_excel($customer_id, $profile_class_ids, $as_of_date){
		...
		//~ $sql = "SELECT hcaa.cust_account_id customer_id,
					...
					//~ FROM IPC.IPC_INVOICE_DETAILS soa
					 ...
					 //~ WHERE     1 = 1
						...
						//~ HAVING SUM((soa.invoice_orig_amount +NVL(adj.adjustment_amount,0)) - NVL (araa.paid_amount, 0)) > 0
					 //~ ORDER BY due_Date nulls last"; echo $sql;
		...
		$sql = "SELECT hcaa.cust_account_id                          customer_id,
						 ...
						 SUM(soa.invoice_orig_amount)                       transaction_amount,
						 SUM(soa.wht_orig_amount)                           wht_amount,
						 ( SUM( soa.invoice_orig_amount)
						  + SUM(NVL (adj.adjustment_amount, 0))
						  - SUM(NVL (araa.paid_amount, 0)))
							balance,
						 soa.invoice_currency_code                     currency,
						 soa.exchange_rate
					FROM IPC.IPC_INVOICE_DETAILS soa
						 ...
						 LEFT JOIN hz_parties hp ON hcaa.party_id = hp.party_id
						 LEFT JOIN
						 (SELECT DISTINCT pdc.check_number, pdc.check_bank, unit.cs_number
							FROM ipc.ipc_treasury_pdc pdc
								 LEFT JOIN ipc.ipc_treasury_pdc_units unit
									ON pdc.check_id = unit.check_id
								 LEFT JOIN ipc.ipc_treasury_approved_pdc app_pdc
