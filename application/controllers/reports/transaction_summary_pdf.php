...
class Transaction_summary_pdf extends CI_Controller {
	...
	public function index(){
		...
		$rows = $this->transaction_model->get_summary_per_customer_pdf($customer_id, oracle_date($as_of_date));
		$customer = $this->transaction_model->get_customer($customer_id);
		...
		$html = '<table style="font-size: 10px;padding: 3px;">
					<tr>
						<td align="left" >'.$customer[0]->CUSTOMER_NAME.'</td>
						<td align="right">As of : '.$as_of_date.'</td>
					</tr>
				</table>';
		...
		$this->pdf->Output('transaction_summary.pdf', 'I');
